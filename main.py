...
from auth import get_db, create_token, get_current_user, SECRET_KEY, ALGORITHM, verify_password, hash_password
...
def start_guess(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    game = Game(user_id=user.id, game_type="guess", secret=str(random.randint(1,100)), lives=7, attempts=0, status="active")
    db.add(game)
    db.commit()
    return {"message":"Number Guess started","lives":game.lives,"attempts":0}
...
def play_guess(data: Guess, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not game: raise HTTPException(status_code=400, detail="No active game")
    game.attempts+=1
    if data.guess==int(game.secret):
        game.status="won"; user.score+=50+game.lives*5; user.wins+=1; db.commit()
        return {"result":"WIN 🎉","attempts":game.attempts,"points":50+game.lives*5,"total_score":user.score}
    game.lives-=1
    if game.lives<=0: game.status="lost"; db.commit(); return {"result":"GAME OVER 💀","number":game.secret}
    db.commit()
    return {"hint":"too low" if data.guess<int(game.secret) else "too high","lives":game.lives,"attempts":game.attempts}

# ================= BULLS & COWS =================
@app.post("/bulls-cows/start")
def start_bulls_cows(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    game = Game(user_id=user.id, game_type="bulls_cows", secret=generate_bulls_cows_number(), lives=10, attempts=0, status="active")
    db.add(game); db.commit()
    return {"message":"Bulls & Cows started","lives":game.lives,"attempts":0}
...
def play_bulls_cows(data: BullsCowsGuess, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if bulls==4: game.status="won"; user.score+=50+game.lives*5; user.wins+=1; db.commit(); return {"result":"WIN 🎉","attempts":game.attempts,"points":50+game.lives*5,"total_score":user.score}
    game.lives-=1
    if game.lives<=0: game.status="lost"; db.commit(); return {"result":"GAME OVER 💀","secret":game.secret}
    db.commit()
    return {"bulls":bulls,"cows":cows,"lives":game.lives,"attempts":game.attempts}
